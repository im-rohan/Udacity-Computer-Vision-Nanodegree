...
class DecoderRNN(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # batch_size,cap_length -> batch_size,cap_length-1,embed_size

         # -> batch_size, caption (sequence) length, embed_size
        inputs = torch.cat((features.unsqueeze(dim=1),embeds), dim=1)

        lstm_out, _ = self.lstm(inputs);   
        # print (lstm_out.shape) -> batch_size, caplength, hidden_size

        # get the scores for the most likely words
        outputs = self.hidden2vocab(lstm_out);     
        # print (outputs.shape) -> batch_size, caplength, vocab_size
        
        return outputs  #[:,:-1,:] 
    # discard the last output of each sample in the batch.
    ...
    def sample(self, inputs, states=None, max_len=20):
        ...
        for i in range(max_len):
            ...
            # batch_size=1, sequence length=1 ->1,1,embedsize
